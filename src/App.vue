...
import { NDialogProvider, NConfigProvider, darkTheme, lightTheme, NTooltip, NBadge } from 'naive-ui'
...
function isUsingDarkTheme() {
  ...
  isDarkTheme.value = !!body?.classList.contains('theme-dark')
}

isUsingDarkTheme()
const themeObserver = new MutationObserver(isUsingDarkTheme)
themeObserver.observe(document.querySelector('body')!, { attributes: true, attributeFilter: ['class'] })

function onShowHomePage() {
  HomeRef.value.show()
  if (newVersion.value) {
    storageHelper.setKey(STORAGE_KEYS.KOOK_HELPER_LITE_PREVIOUS_VERSION, version)
...
<template>
  <n-dialog-provider>
    <n-badge :show="newVersion" dot :offset="[-3, 3]" type="success" :color="newVersion ? '' : 'unset'" processing>
      <n-tooltip trigger="hover">
        <template #trigger>
          <svg class="home-icon" @click="onShowHomePage" xmlns="http://www.w3.org/2000/svg"
